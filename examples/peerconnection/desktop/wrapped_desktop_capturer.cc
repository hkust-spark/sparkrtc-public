...
WrappedDesktopCapturer::WrappedDesktopCapturer() : dc_(nullptr), start_flag_(false) {}

bool WrappedDesktopCapturer::Init(size_t target_fps, size_t capture_screen_index) 
{
	// 窗口
  ...
  RTC_LOG(LS_INFO) << "Init WrappedDesktopCapturer finish window_title = " << window_title_ << " , fps = " << fps_ <<"";
  // Start new thread to capture
  return true;
}

WrappedDesktopCapturer* WrappedDesktopCapturer::Create(size_t target_fps,
  ...
  if (!dc->Init(target_fps, capture_screen_index)) {
    RTC_LOG(LS_WARNING) << "Failed to create WrappedDesktopCapturer(fps = "
                        << target_fps << ")";
    return nullptr;
  }
  return dc.release();
...
void WrappedDesktopCapturer::OnCaptureResult(
