...
void GtkMainWnd::SwitchToPeerList(const Peers& peers) {
  ...
  for (Peers::const_iterator i = peers.begin(); i != peers.end(); ++i){
    // log peer id and name
    RTC_LOG(LS_INFO) << __FUNCTION__ << " peer id: " << i->first << " name: " << i->second;
...
void GtkMainWnd::OnClicked(GtkWidget* widget) {
  ...
  // hxj: We don't need to specify by widget, since its autocalled.
...
void GtkMainWnd::OnRowActivated(GtkTreeView* tree_view,
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    ...
    // log id
    RTC_LOG(LS_INFO) << __FUNCTION__ << " peer id: " << id;
    if (id != -1)
